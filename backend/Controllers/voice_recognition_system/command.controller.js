...
//const mqttService = require("../../services/deviceService");


const addCommand = async (req, res) => {
  try {
    const { commandText, commandType } = req.body;

    if (!commandText || !commandType) {
      return res.status(400).json({ message: "Vui lòng cung cấp nội dung câu lệnh." });
    }
    const exists = await Command.findOne({
      commandText,
      commandType,
    });
    if (exists) {
      return res
        .status(409)
        .json({ message: "Lệnh này đã tồn tại trong hệ thống." });
    }

    const newCommand = new Command({ commandText, commandType });
  ...
  try {
    ...
    if (!deletedCommand) {
      return res.status(404).json({
        message: "Không tìm thấy câu lệnh để xóa."
      });
    }

    res.status(200).json({
      message: "Xóa câu lệnh thành công!",
      deletedCommand
    });
  } catch (error) {
    res.status(500).json({
      message: "Lỗi khi xóa câu lệnh.",
      error: error.message
    });
  }
};
...
module.exports = { addCommand, getCommands, compareTranscriptAndPublish, getCommandsFromDB, deleteCommand };
